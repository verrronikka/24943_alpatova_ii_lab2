...
Задача заключается в разработке модели машинного обучения для сегментации персонажей мультсериала «Симпсоны» на основе их визуальных характеристик из набора данных Massachusetts Roads Dataset. Целью является автоматическое определение имени персонажа по изображению с максимизацией метрики Accuracy.
